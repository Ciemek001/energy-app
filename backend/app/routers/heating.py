...
from app.db.crud_heating import crud_heating
from app.schemas.heating import HeatingCreate, HeatingOut

router = APIRouter(prefix="/heating", tags=["Heating Systems"])


@router.get("/", response_model=list[HeatingOut])
async def get_heating_systems(db: AsyncSession = Depends(get_db)):
    return await crud_heating.get_all(db)


@router.get("/{system_id}", response_model=HeatingOut)
...
@router.post("/", response_model=HeatingOut)
async def create_heating(system: HeatingCreate, db: AsyncSession = Depends(get_db)):
